...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ["create", "update", "partial_update", "destroy"]:
            ...
            self.permission_classes = []   # hamma uchun ruxsat
        return super().get_permissions()

    def perform_create(self, serializer):
        serializer.save(seller=self.request.user)

#ixtiyoriy foydalanuvchi un faqat nashr etilgan mahsulotlarni korsatish, admin hammasini korishi un
    def get_queryset(self):
        qs = super().get_queryset()
        if self.request.user.is_authenticated and self.request.user.user_role == "admin":
            return qs   # admin hammasini koradi
        #ommaviy royxat un faqat elon qilingan mahsulotni korsatish
        if self.action == "list":
            return qs.filter(status="published")
        return qs   # agar egasi bolsa hamamsini korsatish
