...
class CustomUser(AbstractUser, BaseModel):
    USER_ROLE = ((ORDINARY_USER, ORDINARY_USER), (SELLER, SELLER), (ADMIN, ADMIN))
    AUTH_TYPE = (
        (VIA_PHONE, VIA_PHONE),
        (VIA_EMAIL, VIA_EMAIL),
    )
    ...
    user_role = models.CharField(
        max_length=250, choices=USER_ROLE, default=ORDINARY_USER
    )
    ...
    photo = models.ImageField(
        upload_to="users/", null=False, blank=True, default="users/default_user.png"
    )
    ...
    def generate_and_hash_password(self):
        ...
        except ValueError:  # bu joyda heshlanmagan bolsa xatolik chiqaradi
    ...
    def create_code(self, verify_type):
        self.verify_codes.filter(verify_type=verify_type, is_used=False).update(
            is_used=True
        )

        code = str(random.randint(1000, 9999))
        CodeVerify.objects.create(code=code, user=self, verify_type=verify_type)
        return code
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            ...
            if self.verify_type == VIA_EMAIL:
                self.expiration_time = current_time + timedelta(
