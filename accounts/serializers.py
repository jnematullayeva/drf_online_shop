...
class VerifyCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not verify_code:
            raise ValidationError({"code": "Tasdiqlash kodi notogri"})

        if verify_code.expiration_time and verify_code.expiration_time < timezone.now():
            raise ValidationError({"code": "Kodning muddati tugagan!"})
...
class ResendCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if active_code:
            raise ValidationError(
                {
                    "message": f"Sizda hali amal qiladigan faol kod bor, amal qilish muddati: {active_code.expiration_time}"
                }
            )
...
class ChangeProfileInfoSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if (
            ...
            raise ValidationError({"password": "parol mos kelmadi"})
        return attrs
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user or not user.check_password(password):
            raise ValidationError({"message": "Elektron pochta/telefon yoki parol notogri"})
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user:
            raise ValidationError({"message": "Foydalanuvchi ttopilmadi"})
        ...
        if not verify_code:
            raise ValidationError({"code": "Tasdiqlash kodi xato kiritildi."})
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user:
            raise ValidationError({"message": "Identifikatsiyani tasdiqlash kerak"})
