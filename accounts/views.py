...
class LogoutView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            token.blacklist()
            return Response(
                {"success": True, "message": "Tizimdan muvaffaqiyatli chiqildi"},
                status=status.HTTP_200_OK,
            )
        except Exception:
            return Response(
                {"message": "Noto'g'ri refresh token"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if verify_type == VIA_EMAIL:
            send_to_mail(
                message=f"Parolni tiklash kodingiz: {code}",
                email=user.email,
            )
        else:
